...
namespace owdin {
    void owdinnetwork::debug( account_name account ) {
        auth_account( account, false );

        const char* ver_info = "v0.0.2/alpha";
        print( "[", ver_info, "] : ", eosio::name{account} );
    }


    void owdinnetwork::create( account_name account, asset maximum_supply ) {
    ...
    void owdinnetwork::issue(account_name account, asset quantity, string memo) {
        owdin_auth::auth_account( account, true );
        ...
        if( account != st.issuer ) {
            SEND_INLINE_ACTION( *this, transfer, {st.issuer,N(active)}, {st.issuer, st.issuer, account, quantity, memo} );
        }
    }

    void owdinnetwork::transfer( account_name account, account_name sender, account_name receiver, asset quantity, string memo ) {
    ...
    void owdinnetwork::sub_balance( account_name owner, asset value, string memo ) {
    ...
    void owdinnetwork::add_balance( account_name owner, asset value, account_name ram_payer, string memo ) {
        accountIndex accounts( _self, owner );
...
EOSIO_ABI( owdin::owdinnetwork, (debug)(create)(issue)(transfer)(reward)(add)(reset)(owdinport)(errlog)(statlog) )
